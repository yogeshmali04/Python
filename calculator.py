...
class Calculator(Tk):
    def __init__(self):
        super().__init__()
        self.title("Rohan Ka Calculator")

        # create screen widget
        self.screen = Text(self, state='disabled', width=50, height=3, background="white", foreground="blue")
        ...
        for row in range(1, 5):
            for column in range(4):
                buttons[count].grid(row=row, column=column,padx = 10, pady = 10)
                count += 1
    ...
    def createButton(self, val, write=True, width=7):
        ...
        return Button(self, text=val, command=lambda: self.click(val, write), width=width)
...
if __name__ == '__main__':
    window = Calculator()
    window.createButton('val')
    window.click('click me', '')
    window.clear_screen()
    window.insert_screen('')
    window.mainloop()